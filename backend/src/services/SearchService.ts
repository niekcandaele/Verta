...
import { SearchRequest, SearchApiRequest, SearchApiResponse } from 'shared-types';
...
export class SearchService {
  ...
  async search(tenantSlug: string, apiRequest: SearchApiRequest): Promise<SearchApiResponse> {
    try {
      ...
      if (error instanceof Error && error.message.includes('Circuit breaker is OPEN')) {
        throw new ApiError(503, 'Service Unavailable', 'Search service is temporarily unavailable');
      }
      
      throw new ApiError(500, 'Internal Server Error', 'Search operation failed');
    }
  }
}